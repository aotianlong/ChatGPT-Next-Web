          ...
          onmessage(msg) {
            ...
            try {
              if (response && !response.isToolMessage) {
                ...
                let name='插件'
                console.log(response.toolName)
                for (const cnplugin of CN_PLUGINS) {
                  if(cnplugin.toolName==response.toolName){
                    name=cnplugin.name
                    break
                  }
                }
                if(response.toolName.includes('search')&&name=='插件'){
                  name='联网引擎'
                }
                if(response.toolName.includes('arxiv')&&name=='插件'){
                  name='文章引擎'
                }
                if(response.toolName.includes('wiki')&&name=='插件'){
                  name='维基百科'
                }
                options.onToolUpdate?.(name, response.message);
              }
            } catch (e) {
